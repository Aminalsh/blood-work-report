...
import io.everyonecodes.spring_module.dto.UpdateReportForm;
...
public class ReportPageController {
    ...
    @GetMapping("/{reportId}")
    public String showReport(@PathVariable Integer reportId, Model model) {
        model.addAttribute("report", reportService.findReportById(reportId));
        return "reports/details";
    }

    @GetMapping("/{reportId}/edit")
    public String showEditForm(@PathVariable Integer reportId, Model model) {
        ...
        for (MarkerResult result : report.getResults()) {
            ...
            if (result.getNumericValue() != null) {
                input.setValue(result.getNumericValue().toPlainString());
            } else {
    ...
    public String updateReport(
        ...
        reportService.updateReport(reportId, submittedResults);
    ...
    private MarkerResultRequest convertInput(
        ...
        if (marker == null) {
            throw new IllegalArgumentException(
                    "Marker does not belong to this report"
            );
        }
